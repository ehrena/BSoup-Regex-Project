...
def find_urls(s):
    #Your code here
    urls = re.findall("http[s]?:\/\/\S+\.\S{2,}", s)
    return urls
...
def grab_headlines():
    url = urllib.request.urlopen("http://www.michigandaily.com/section/opinion")
    #f = open('opinion.html')
    readList = []
    soup = BeautifulSoup(url, "html.parser") 
    mostRead = soup('ol')
    headlines = []
    for litag in mostRead:
        headlines.append(litag.text.split("\n")[:-1])
    return headlines[0] #the split creates one list item within another list, so indexing at position zero just extracts that item
...
## End with this page: https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All12 
## INPUT: N/A. No input.
...
def get_umsi_data():
    ...
    while page<=12:
        ...
        soup = BeautifulSoup(url, "html.parser")
        myNames = soup.findAll('div', {'class': "field field-name-title field-type-ds field-label-hidden"})
        myTitles = soup.findAll('div', {'class': "field field-name-field-person-titles field-type-text field-label-hidden"}) 
        for i in range(20):
            try:
                umsi_titles[myNames[i].text] = myTitles[i].text
            except IndexError: #on last page, there are only 12 faculty members, so this makes sure we don't go out of range
                pass
        page+=1
    return umsi_titles
    ...
    #<div class="field field-name-field-person-titles field-type-text field-label-hidden">
